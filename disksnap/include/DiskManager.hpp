...
private:
    DiskManager(int DiskNum, int DiskVolume, int HeadToken, std::vector<int>& label_index);

public:
    static DiskManager &getInstance(int DiskNum, int DiskVolume, int HeadToken, std::vector<int>& label_index); // 传入磁盘数 磁盘存储空间大小 磁头移动速度
    static DiskManager &getInstance();

    std::vector<Disk*> get_disks(std::vector<int> disk_ids);
    Disk& get_disk(int disk_id);
    void store_obj(int id, int size, int tag, const std::vector<int>& label_index); // 修改存储函数接口
