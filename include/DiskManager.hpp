...
private:
    DiskManager(int DiskNum, int DiskVolume, int HeadToken, std::vector<double> &tag_ratio);

public:
    static DiskManager &getInstance(int DiskNum, int DiskVolume, int HeadToken, std::vector<double> &tag_ratio); // 传入磁盘数 磁盘存储空间大小 磁头移动速度
